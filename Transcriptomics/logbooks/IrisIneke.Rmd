...
**BELANGRIJK:**\
Later erachter gekomen dat ze in de NSG-muizen toch geen menselijke
cellen hebben gestopt. Ze hebben enkel een slechter immuunsysteem.

## Quality control:

(woensdag 16 oktober) FastQC en MultiQC uitgevoerd op de data (duurt erg
lang, rekening mee houden in planning).
...
De adapter content is erg hoog. Deze proberen te verminderen met
headcrop (begin eraf halen).
...
Trimmomatic](https://pubmed.ncbi.nlm.nih.gov/33575617/) hoeven te
gebruiken (data is al goed genoeg).

## Mapping

We zijn erachter gekomen dat in de NSG muizen geen menselijke cellen
zitten. Dus deze moeten we ook mappen tegen het muis-referentie-genoom
(Wat wel heel gek is is dat in het artikel staat dat ze alles tegen het
menselijke-referentie-genoom hebben gemapt, is dit een fout ?).\
Code van
[Ramon](https://github.com/RamonReilman/GenomicsTranscriptomics/blob/main/Transcriptomics/logbooks/RamonReilman.Rmd)
...
De mapping tegen het muis genoom ziet er beter uit, dus waarschijnlijk
zit er een fout in het artikel.

## Deseq

(30/31 oktober) De volgende code is van
[Ramon](https://github.com/RamonReilman/GenomicsTranscriptomics/blob/main/Transcriptomics/logbooks/RamonReilman.Rmd)
(Zie ook Teams):
...
Het annoteren van onze data zorgt ervoor dat de genen de NCBI naamgeving
krijgen. Zo is het straks overzichtelijker om informatie te vinden over
de genen.
...
In de bestanden kon niks gevonden worden over de genen, maar zo is het
wel duidelijker waar elke kolom voor staat. Dat moet nu nog toegepast
worden in de counts df
...
Nu zijn de SRR\* namen vervangen met waar ze voor staan en is de df
duidelijker te lezen, om straks makkelijker de kolommen op te halen die
horen bij elk mogelijke variant groepeer ik deze.
...
Nu kunnen deze variabelen gebruikt worden om de juiste kolommen te
selecteren.

Dan kan dit nu samengevoegd worden met DESEQ, wat Janine heeft
uitgezocht. Bekijk [Janine haar
logboek](https://github.com/RamonReilman/GenomicsTranscriptomics/blob/main/Transcriptomics/logbooks/JaninePostmus.Rmd)
voor meer informatie over de code.
...
Nu dit allemaal is uitgevoerd kunnen we verder met een MA plot te maken
van nsg-baseline vs nsg-entinostat door gebruik te maken van de 'res'
die is gemaakt. Hierbij heb ik
een[tutorial](https://bioconductor.org/packages/devel/bioc/vignettes/DESeq2/inst/doc/DESeq2.html#ma-plot)
gebruikt.
...
De plot bevat nog noise, dus we gaan de 'Log fold change shrinkage for
visualization and ranking' uitvoeren van de tutorial.
...
Bij coef wordt maar 1 input geaccepteerd, dus eerst de twee namen apart
invullen van de situaties die we willen visualiseren:

Maar nu is de vraag hoe we dit samen moeten voegen ? Er een vector van
maken werkt niet.

[Ramon](https://github.com/RamonReilman/GenomicsTranscriptomics/blob/main/Transcriptomics/logbooks/RamonReilman.Rmd)
heeft een subset gemaakt gemaakt van de twee situaties, dit is handig om
alleen de informatie van de situaties weer te geven in de plot:
...
Hiermee doorgaan: Plot van alleen de baseline en Entinostat:
...
Interpretatie plot: x-as is de counts en de y-as is de log verandering.\
Dus hoe verder op de x-as hoe meer counts van het gen is. En hoe verder
op y-as hoe meer verschillend de expressie van het gen is.\
De genen die significant verschillen in expressie zullen dus de blauwe
bolletjes zijn aan de rechterkant van de plot.

[Ramon](https://github.com/RamonReilman/GenomicsTranscriptomics/blob/main/Transcriptomics/logbooks/RamonReilman.Rmd)
heeft het probleem met de enkele input van lfcShrink opgelost:
...
Dit lost het probleem op dat uit de resultsNames() eerder alle situaties
apart kwamen. Nu zijn ze verdeeld in de situaties die wij wel willen
gebruiken (treatment_Entinostat_vs_Baseline) en die wij niet willen
gebruiken (Intercept) voor deze plot.\
...
Dit is echt veel beter. Het is interessant om te kijken welke genen bij
welke punten horen, dus we gaan proberen om de genen erbij te zetten.
Hiervoor is een [online
artikel](https://www.r-bloggers.com/2016/01/repel-overlapping-text-labels-in-ggplot2/)
gebruikt. Eerst de plot in ggplot maken:
...
In de bovenstaande plot zie je de MAplot die we eerder ook hadden, maar
dan in ggplot2. Ook zijn de genen toegevoegd. De padj \< 0.05 staat voor
de p-waarde. Alles wat kleiner dan 0.05 is statistisch significant en
wordt verder gebruikt. Dus alleen alles met een p-waarde kleiner dan
0.05 wordt een gen bijgezet (en blauw gekleurd).\
Zoals eerder vermeld is, zijn de blauwe punten rechts in de grafiek het
interessants, omdat die genen significant verschillen in expressie.\

'Gene-Eef 1 a 1' staat het meest rechts. Dus dit is een interessant gen.
Wat is de functie van dit gen?\
Dit gen helpt om aminozuur-gebonden tRNAs naar het ribosoom te brengen
voor de eiwitsynthese. Als dit gen minder tot expressie komt, kan dat
dus tot een verminderde eiwitsynthese leiden. Met een verminderde
eiwitsynthese kan de celdeling langzamer gaan. Met een verminderde
celdeling wordt schade minder snel gerepareerd. Ook werkt het
immuunsysteem minder goed, aangezien er minder snel nieuwe cellen
aangemaakt kunnen worden (zoals witte bloedcellen).\
[Bron: Gene-Eef 1 a 1](https://www.ncbi.nlm.nih.gov/gene/1915)\

Conclusie voor onze situatie:\
In de plot is de NSG-Entinostat en de NSG-Baseline met elkaar
vergeleken. De y-waarde is boven de 0, dus de gen-expressie is hoger in
de eerste situatie. Dat is in dit geval de baseline. Dus de
gen-expressie is hoger bij de baseline dan bij de Entinostat (meer
eiwitsynthese bij baseline). Dus de Entinostat zorgt er mogelijk voor
dat dit gen minder tot expressie komt. Wat zou suggereren dat de
Entinostat ervoor zorgt dat de eiwitsynthese verminderd en hiermee de
celdeling vertraagd.\

(Wat misschien interessant is: De isovorm (alfa 2) wordt geïdentificeerd
als een auto-antigeen bij 66% van de patiënten met het Felty-syndroom.
(Auto-antigen: antigen van je eigen lichaam dat je lichaam aanvalt,
omdat het herkent wordt als vreemd). Dus de isovorm kan ook direct
invloed hebben op het immuunsysteem.)

## Gen expressie vehicle vraag

Uit
[Ramon](https://github.com/RamonReilman/GenomicsTranscriptomics/blob/main/Transcriptomics/logbooks/RamonReilman.Rmd)
zijn plot bleek dat bij de NSG muis de vehicle een hoge gen-expressie
had op het gene-Ces4a. Dit gen zorgt voor de afbraak van xenobiotische
stoffen. Onze theorie is dat de stof die de vehicle toegediend heeft
gekregen een xenobiotische stof is, waardoor de vehicle een hoge
expressie heeft van gene-Ces4a. In [het
artikel](https://pmc.ncbi.nlm.nih.gov/articles/PMC8363284/) staat de
volgende zin: 'BBN963 cells were treated with DMSO or 1 μM entinostat
for 72 hours in vitro.' Dit betekend dat de de stof waarschijnlijk
[DMSO](https://pubmed.ncbi.nlm.nih.gov/17177172/) is. Dit is een
xenobiotische stof, dus dit kan de hoge gen-expressie verklaren.

## Annotatie met biomart

Geprobeerd met
[Jasper](https://github.com/RamonReilman/GenomicsTranscriptomics/blob/main/Transcriptomics/logbooks/JasperJonker.Rmd)
en
[Ramon](https://github.com/RamonReilman/GenomicsTranscriptomics/blob/main/Transcriptomics/logbooks/RamonReilman.Rmd).
Onze dataset bevat teveel genen en de library connectie werkt niet, dus
hebben we besloten om geen annotatie uit te voeren.

## Venn diagram eerste poging, klopt niet helemaal:

[Bron voor
venndiagram](https://rpubs.com/Nishapaudel/DESeq2_sex_wise_dfs)
...
Verdelen in C57BL6-muis en NSG-muis (er wordt een genen-count per kolom
aangemaakt).
...
Dit gaat niet helemaal goed. Diagram nodig van elke treatment apart van
soort 1 tegen soort 2. De code is steeds hetzelfde, maar dan met de
juiste treatment (entinostat, vehicle, baseline).

## Venndiagram entinostat:

Verdelen in C57BL6-muis en NSG-muis voor alleen de entinostat groep (er
wordt een genen-count per kolom aangemaakt).
...
Echt heel raar, maar hij geeft van elke kolom alles 2 keer. Met exact
dezelfde cijfers en namen.
...
Maar uit de tests komen dat er geen dubbele kolom namen zijn en dat de
namen die ik heb ingevoerd kloppen ?

```{r tweede_test_dubbele_kolomnamen}
print(colnames(data_rearranged_C57BL6_entinostat))
```

En de kolom namen kloppen ook, dus ik neem aan dat het een fout is in
het weergeven (Bij alle andere gaat het wel goed).
...
In de venndiagrammen zie je de upregulated genen weergegeven van de
Entinostat groepen. De genen die in de overlap staan, zijn significant
upregulated in beide muizen, terwijl de genen in de aparte delen van de
Venn-cirkel alleen in één van de twee muizen significant upregulated
zijn.

## Venndiagram Vehicle:

Verdelen in C57BL6-muis en NSG-muis voor alleen de vehicle groep (er
wordt een genen-count per kolom aangemaakt).
...
In de venndiagrammen zie je de upregulated genen weergegeven van de
Vehicle groepen. De genen die in de overlap staan, zijn significant
upregulated in beide muizen, terwijl de genen in de aparte delen van de
Venn-cirkel alleen in één van de twee muizen significant upregulated
zijn.
...
Verdelen in C57BL6-muis en NSG-muis voor alleen de baseline groep (er
wordt een genen-count per kolom aangemaakt).
...
In de venndiagrammen zie je de upregulated genen weergegeven van de
Baseline groepen. De genen die in de overlap staan, zijn significant
upregulated in beide muizen, terwijl de genen in de aparte delen van de
Venn-cirkel alleen in één van de twee muizen significant upregulated
zijn.
...
Er is de meeste overlap bij vehicle(16739) en baseline(16542)(Entinostat
heeft een overlap van 15457). Waarin de vehicle(16739) de meeste overlap
heeft.
...
X-as:\
Log2 fold change laat zien hoe groot het verschil van gen-expressie is
tussen de baseline en entinostat.\
Alles aan de rechterkant: de gen-expressie is hoger bij baseline dan bij
entinostat.\
Alles aan de linkerkant: de gen-expressie is hoger bij de entinostat dan
bij baseline.\
Y-as:\
-Log10(p-waarde) laat zien hoe hoog de significantie is in het verschil
tussen de gen-expressie (hoe hoger hoe meer significantie).

Het lijkt me dus interessant om te kijken naar de linkerkant. Want ik
wil kijken welke genen entinostat mogelijk tot een hogere expressie
brengt. (Baseline is totdat de tumor een bepaalde grootte heeft.
Entinostat is als na die grootte entinostat is gegeven). De meest
interessante (hoog expressie verschil en hoge significantie) is
gene-Gm46987, maar daar kan ik niet veel over vinden. Welke ook
interessant is, is gene-Dcstamp. Dit gen reguleert Dendritische
cellen(soort witte bloedcellen). En zorgt er dus voor dat het
immuunsysteem weet welke cellen deze moet aanvallen (en ook zichzelf
niet aanvalt). Dat de expressie van dit gen hoger is nadat de tumor
groter is en behandeld wordt met entinostat is dus een goed teken. Het
kan dus betekenen dat entinostat ervoor zorgt dat de gen-expressie van
dit gen hoger wordt. Maar het kan ook betekenen dat de tumor eerst nog
te klein was(bij de baseline) om de gen-expressie van dit gen te
verhogen. Je zou hiervoor eigenlijk naar het verschil van de
gen-expressie moeten kijken van de vehicle(niet behandeld na bepaalde
grootte tumor) en de entinostat. Dan kan je beter onderzoeken of de
expressie van dit gen hoger wordt door de entinostat of meer door de
groeiende tumor. Want als dit zo is, dan kan entinostat een goede impact
hebben als behandeling op de tumor.

[Bron gene-Dcstamp](https://www.uniprot.org/uniprotkb/Q9H295/entry)

## Herkansing

Eerst gaan we de subgroepen opnieuw doen. De handigere manier is
namelijk met `results`.\
Eerst kijken we wat de namen zijn van de verschillende groepen:

```{r}
levels(dds$source_name)
```

Nu kunnen we de vergelijkingen maken:

```{r res_entinostat_C57BL6}
res_entinostat_C57BL6 <- results(dds, 
                           contrast = c("source_name", "C57BL/6_bladder tumor_Entinostat", "C57BL/6_bladder tumor_Baseline"),
                                        alpha = 0.05)
```

Met alpha is de p-waarde op 0.05 gezet. Hierboven is de entinostat en de
baseline van de C57BL/6 muizen vergeleken.\
De samenvatting hiervan:

```{r}
summary(res_entinostat_C57BL6)
```

In de summary resultaten kun je aflezen dat er 31860 reads zijn
meegenomen in de results. Ook staat erin dat de p-value inderdaad verzet
is naar 0.05. \
Verder hebben 6300 genen (wat 20% is) een hogere
gen-expressie in de entinostat dan de baseline. En 3776 genen (wat 12%
is) een lagere gen-expressie in de entinostat dan de baseline. \
Verder is
er 0.52% van de genen outliers en worden er 8508 (wat 27% is) van de
genen genegeerd door een te laag gemiddeld aantal reads.\

Nu gaan we hetzelfde doen voor de andere groepen:

```{r res_vehicle_C57BL6}
res_vehicle_C57BL6 <- results(dds, 
                           contrast = c("source_name", "C57BL/6_bladder tumor_Vehicle", "C57BL/6_bladder tumor_Baseline"),
                                        alpha = 0.05)
```

Hierboven is de vehicle en de baseline van de C57BL/6 muizen vergeleken
van de C57BL/6 muizen. \
De samenvatting hiervan:

```{r}
summary(res_vehicle_C57BL6)
```

In de summary resultaten kun je aflezen dat er 31860 reads zijn
meegenomen in de results. Ook staat erin dat de p-value inderdaad verzet
is naar 0.05. \
Verder hebben 31 genen (wat 0.097% is) een hogere
gen-expressie in de vehicle dan de baseline, wat veel lager is dan bij
de vehicle. En 20 genen (wat 0.063% is) een lagere gen-expressie in de
vehicle dan de baseline, wat ook veel lager is dan de vehicle. \
Verder is
er 0.52% van de genen outliers en worden er 10937 (wat 34% is) van de
genen genegeerd door een te laag gemiddeld aantal reads. Deze waarde is
veel hoger dan de entinostat. \

Dus in de C57BL/6 muizen hebben meer genen een hogere gen-expressie met
entinostat dan de vehicle. En ook zijn er wat meer low counts die niet
meegeteld worden bij de vehicle. \

En nu voor de NSG muizen:

```{r res_entinostat_NSG}
res_entinostat_NSG <- results(dds, 
                           contrast = c("source_name", "NSG_bladder tumor_Entinostat", "NSG_bladder tumor_Baseline"),
                                        alpha = 0.05)
```

Hierboven is de entinostat en de baseline van de NSG muizen vergeleken. \
De samenvatting hiervan:

```{r}
summary(res_entinostat_NSG)
```

In de summary resultaten kun je aflezen dat er 31860 reads zijn
meegenomen in de results. Ook staat erin dat de p-value inderdaad verzet
is naar 0.05. \
Verder hebben 317 genen (wat 0.99% is) een hogere
gen-expressie in de entinostat dan de baseline. En 105 genen (wat 0.33%
is) een lagere gen-expressie in de entinostat dan de baseline. \
Verder is
er 0.52% van de genen outliers en worden er 12146 (wat 38% is) van de
genen genegeerd door een te laag gemiddeld aantal reads. Dit zijn
allemaal lage percentages (behalve de outliers, die zijn dan weer hoger
dan eerder). \

Nu de andere groep:
```{r res_vehicle_NSG}
res_vehicle_NSG <- results(dds, 
                           contrast = c("source_name", "NSG_bladder tumor_Vehicle", "NSG_bladder tumor_Baseline"),
                                        alpha = 0.05)
```

Hierboven is de vehicle en de baseline van de NSG muizen vergeleken. \
De
samenvatting hiervan:

```{r}
summary(res_vehicle_NSG)
```

In de summary resultaten kun je aflezen dat er 31860 reads zijn
meegenomen in de results. Ook staat erin dat de p-value inderdaad verzet
is naar 0.05. \
Verder hebben 734 genen (wat 2.3% is) een hogere
gen-expressie in de vehicle dan de baseline, wat juist weer hoger is dan
bij de vehicle.Dat was bij de andere muis juist andersom (daar was de
entinostat hoger dan de vehicle). En 338 genen (wat 1.1% is) een lagere
gen-expressie in de vehicle dan de baseline, wat ook weer hoger is dan
de vehicle. \
Verder is er 0.52% van de genen outliers en worden er 13960
(wat 44% is) van de genen genegeerd door een te laag gemiddeld aantal
reads. Deze waarde is dan weer hoger dan de entinostat. \

Dus in de NSG muizen hebben meer genen een hogere gen-expressie met
vehicle dan de entinostat vergeleken met de baseline. En ook zijn er wat
meer low counts die niet meegeteld worden in de vehicle. \

Dus de vergelijking tussen de twee muizen is: Bij beide muizen is de low
counts hoger bij de vehicle dan de entinostat. Maar bij de NSG muizen is
de gen-expressie hoger bij de vehicle dan bij de entinostat vergeleken
met de baseline. Terwijl dit bij de C57BL/6 muizen precies andersom is. \

Er zit dus een duidelijk verschil in de twee soorten muizen. \
Het lijkt nu alsof de entinostat beter werkt bij de C57BL/6 muizen (de muizen met
een beter immuunsysteem dan de NSG muizen). En de entinostat niet echt
aanslaat bij de NGS muizen (de muizen met een minder goed immuunsysteem
dan de C57BL/6 muizen).

### plotjes opnieuw (venn diagram)
Eerst moeten we de up en down gereguleerde genen selecteren. Dit doen we met de results uit deseq:
Dit doen we voor:
- Entinostat C57
- Vehicle C57
- Entinostat NSG
...
```{r}
# Voor Entinostat C57
res_df_entC57 <- as.data.frame(res_entinostat_C57BL6)
up_ent_C57 <- rownames(res_df_entC57[which(res_df_entC57$log2FoldChange > 1 & res_df_entC57$padj < 0.05), ])
down_ent_C57 <- rownames(res_df_entC57[which(res_df_entC57$log2FoldChange < -1 & res_df_entC57$padj < 0.05), ])
```


```{r}
# Voor Vehicle C57
res_df_vehC57 <- as.data.frame(res_vehicle_C57BL6)
up_veh_C57 <- rownames(res_df_vehC57[which(res_df_vehC57$log2FoldChange > 1 & res_df_vehC57$padj < 0.05), ])
down_veh_C57 <- rownames(res_df_vehC57[which(res_df_vehC57$log2FoldChange < -1 & res_df_vehC57$padj < 0.05), ])
```


```{r}
# Voor Entinostat NSG
res_df_entNSG <- as.data.frame(res_entinostat_NSG)
up_ent_NSG <- rownames(res_df_entNSG[which(res_df_entNSG$log2FoldChange > 1 & res_df_entNSG$padj < 0.05), ])
down_ent_NSG <- rownames(res_df_entNSG[which(res_df_entNSG$log2FoldChange < -1 & res_df_entNSG$padj < 0.05), ])
```
log2FoldChange > 1: bij entinostat vs baseline moet de vehicle bij up regulated mininmaal 2x meer expressie hebben dan de baseline.

```{r}
# Voor Vehicle NSG
res_df_vehNSG <- as.data.frame(res_vehicle_NSG)
up_veh_NSG <- rownames(res_df_vehNSG[which(res_df_vehNSG$log2FoldChange > 1 & res_df_vehNSG$padj < 0.05), ])
down_veh_NSG <- rownames(res_df_vehNSG[which(res_df_vehNSG$log2FoldChange < -1 & res_df_vehNSG$padj < 0.05), ])
```


Nu we de up en down regulated genen hebben geselecteerd, kunnen we verder met de venn diagrams maken.
Eerst gaan we venn diagrams maken van het verschil tussen de twee muizen in entinostat en vehicle:
- C57 vs NSG (Entinostat)	up
- C57 vs NSG (Entinostat)	down
- C57 vs NSG (Vehicle)	up
- C57 vs NSG (Vehicle)	down
```{r}
# Up genen voor C57 en NSG
venn.plot_up_C57vsNSG_entinostat <- draw.pairwise.venn(
  area1 = length(up_ent_C57),
  area2 = length(up_ent_NSG),
  cross.area = length(intersect(up_ent_C57, up_ent_NSG)),
  category = c("C57BL/6 Entinostat", "NSG Entinostat"),
  fill = c("blue", "red")
)
```

```{r plot_up_C57vsNSG_entinostat}
grid.newpage()
grid.draw(venn.plot_up_C57vsNSG_entinostat)
```
Je ziet in plot dat er bij de entinostat de C57BL/6 (5143) aanzienlijk meer genen geactiveerd worden dan bij de NSG(156) muis. Hierbij komen 44 genen overeen. 


```{r}
# vehicle vergelijking
venn.plot_up_C57vsNSG_vehicle <- draw.pairwise.venn(
  area1 = length(up_veh_C57),
  area2 = length(up_veh_NSG),
  cross.area = length(intersect(up_veh_C57, up_veh_NSG)),
  category = c("C57BL/6 Vehicle", "NSG Vehicle"),
  fill = c("green", "yellow") # andere kleurtjes :)
)
```

```{r plot_up_C57vsNSG_vehicle}
grid.newpage()
grid.draw(venn.plot_up_C57vsNSG_vehicle)
```
Je ziet in plot dat er ook bij de vehicle in de NSG (297) aanzienlijk meer genen geactiveerd worden dan bij de C57BL/6 (27) muis. Hierbij komen geen genen overeen. Dit is precies andersom dan bij de entinostat; daar waren de NSG up regelated genen juist minder dan in de C57BL/6.


```{r}
# down genen voor C57 en NSG
venn.plot_down_C57vsNSG_entinostat <- draw.pairwise.venn(
  area1 = length(down_ent_C57),
  area2 = length(down_ent_NSG),
  cross.area = length(intersect(down_ent_C57, down_ent_NSG)),
  category = c("C57BL/6 Entinostat", "NSG Entinostat"),
  fill = c("blue", "red")
)
```

```{r plot_down_C57vsNSG_entinostat}
grid.newpage()
grid.draw(venn.plot_down_C57vsNSG_entinostat)
```

Je ziet in plot dat er bij de entinostat de C57BL/6 (2267) aanzienlijk meer genen gedeactiveerd worden dan bij de NSG(71) muis. Hierbij komen 14 genen overeen.

```{r}
venn.plot_down_C57vsNSG_vehicle <- draw.pairwise.venn(
  area1 = length(down_veh_C57),
  area2 = length(down_veh_NSG),
  cross.area = length(intersect(down_veh_C57, down_veh_NSG)),
  category = c("C57BL/6 Vehicle", "NSG Vehicle"), 
  fill = c("green", "yellow")
)
```

```{r plot_down_C57vsNSG_vehicle}
grid.newpage()
grid.draw(venn.plot_down_C57vsNSG_vehicle)
```

Je ziet in plot dat er bij de vehicle de NSG (151) aanzienlijk meer genen gedeactiveerd worden dan bij de C57BL/6 (18) muis. Hierbij komen geen genen overeen. dit is net zoals bij de up regulated genen weer andersom als de enitnostat.


Nu gaan we venn diagrams maken met het verschil tussen entinostat en vehicle in beide soorten (de treatments):
- Entinostat vs Vehicle binnen C57, up
- Entinostat vs Vehicle binnen C57, down
- Entinostat vs Vehicle binnen NSG, up	
- Entinostat vs Vehicle binnen NSG, down


```{r}
# Entinostat vs Vehicle binnen C57, up
plot_up_C57_treatment <- draw.pairwise.venn(
  area1 = length(up_ent_C57),
  area2 = length(up_veh_C57),
  cross.area = length(intersect(up_ent_C57, up_veh_C57)),
  category = c("Entinostat", "Vehicle"),
  fill = c("darkgreen", "orange") # weer andere kleurtjes
)
```

```{r plot_up_C57_treatment}
grid.newpage()
grid.draw(plot_up_C57_treatment) # moet hier zonder venn.
```
Je ziet in de plot dat in de C57BL/6 er aanzienlijk meer genen up regulated zijn bij entinostat (5165) dan bij vehicle (5). Verder komen er 22 genen overeen.

```{r}
# Entinostat vs Vehicle binnen C57, down
plot_down_C57_treatment <- draw.pairwise.venn(
  area1 = length(down_ent_C57),
  area2 = length(down_veh_C57),
  cross.area = length(intersect(down_ent_C57, down_veh_C57)),
  category = c("Entinostat", "Vehicle"),
  fill = c("darkgreen", "orange")
)
```

```{r plot_down_C57_treatment}
grid.newpage()
grid.draw(plot_down_C57_treatment) # moet hier zonder venn.
```
Je ziet in de plot dat in de C57BL/6 er aanzienlijk meer genen down regulated zijn bij entinostat (2277) dan bij vehicle (14). Verder komen er 4 genen overeen.


```{r}
# Entinostat vs Vehicle binnen NSG, up
plot_up_NSG_treatment <- draw.pairwise.venn(
  area1 = length(up_ent_NSG),
  area2 = length(up_veh_NSG),
  cross.area = length(intersect(up_ent_NSG, up_veh_NSG)),
  category = c("Entinostat", "Vehicle"),
  fill = c("purple", "yellow") # weer andere kleurtjes
)
```

```{r plot_up_NSG_treatment}
grid.newpage()
grid.draw(plot_up_NSG_treatment) # moet hier zonder venn.
```
Je ziet in de plot dat in de NSG er meer genen up regulated zijn bij vehicle (198) dan bij entinostat (101). Verder komen er 99 genen overeen.


```{r}
# Entinostat vs Vehicle binnen NSG, down
plot_down_NSG_treatment <- draw.pairwise.venn(
  area1 = length(down_ent_NSG),
  area2 = length(down_veh_NSG),
  cross.area = length(intersect(down_ent_NSG, down_veh_NSG)),
  category = c("Entinostat", "Vehicle"),
  fill = c("purple", "yellow")
)
```

```{r plot_down_NSG_treatment}
grid.newpage()
grid.draw(plot_down_NSG_treatment) # moet hier zonder venn.
```
Je ziet in de plot dat in de NSG er meer genen up regulated zijn bij vehicle (123) dan bij entinostat (57). Verder komen er 28 genen overeen.

### Conclusie:
Entinostat zorgt dus voor meer gen-veranderingen in C57 dan in NSG muizen. Dit geldt voor up en down regulated genen.
Dus zal entinostat een redelijk werkend immuunsysteem moeten hebben (wat NSG muizen niet hebben en C57 muizen wel).
