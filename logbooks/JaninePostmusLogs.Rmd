...
gemaakt met het zoeken naar een wetenschappelijk artikel. We wouden
graag een onderzoek wat wel te repliceren is, maar wat ook nog een
stukje heeft wat we extra kunnen doen.
...
interessant onderwerp vind. De entinostat werkt heel erg op genexpressie
en ook dit vind ik interessant.
...
maken, Jasper gaat de workflow maken, Ramon zet het plan van aanpak en
de presentatie in elkaar.
...
heeft Ramon meegedaan met de les en het voor onze data uitgevoerd.
...
De verdeling van de tools is alsvolgt: - BWA - Mapping --\> Jasper -
Picard - Duplicates --\> Iris - Strelka - Variant calling --\> Ramon -
SnpEff - Variant annotation --\> Janine

De volgende tools hebben we allemaal geleerd en uitgevoerd: - Seqkit -
Making testdata - Fastqc - Making data quality rapports - Multiqc -
Making data quality rapports from multiple fastqc rapports -
Trimmomatic - Trimming data to enhance data quality

Om te leren hoe we moeten omgaan met de data en testdata heb ik met
behulp van de tool seqkit testdata gemaakt voor ons project. Ik heb dit
eerst uitgevoerd op 1 bestand om te leren hoe het werkt. Omdat onze
...
en zip format in 1 map te krijgen. Omdat dit niet prettig leest en
...
werkt en meer over de tool leren.

**27-09-2024** Ik ben begonnen met het nalezen en aanvullen van mijn
...
**29-09-2024** Aan het testen gegaan met snpEff op een testdata set.
...
Notes: Het command moet uitgevoerd worden vanuit de map waar de tool in
staat.\
...
worden. Ons reference genome van de muis is GRCm39. Bij het downloaden
van de referencegenome is de volgende foutmelding gekomen: FATAL ERROR:
...
mogelijke oplossing bied. Ik ga de volgende stappen uitvoeren vanaf de
link: <https://github.com/pcingola/SnpEff/issues/536>\
...
and rename the files: Mus_musculus.GRCm39.dna.primary_assembly.fa.gz:
...
is. Via dit bericht <https://github.com/pcingola/SnpEff/issues/429> is
...
Ik heb van deze link op het ncbi
<https://www.ncbi.nlm.nih.gov/datasets/genome/GCF_000001635.27/> de
...
Ik heb voor nu besloten om hulp te vragen bij de docenten.
