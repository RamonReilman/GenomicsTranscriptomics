...
<!-- TODO ref naar jaspers logboek --> Eerst heb ik Jasper zijn logboek gevolgd om een index te maken, en de fastq files te mappen naar het referentie genoom.
...
Dit is een groot probleem omdat de erfbare mutaties nu ook gecalled zullen worden, en we mogelijke honderd duizenden mutaties krijgen.
Om vervolgens dan de tumor mutaties vinden is lastig, mogelijk zelfs onmogelijk in de korte tijd die wij hebben.

Ik heb de schrijvers van het artikel gemailed, en zij zijn nu aan het kijken of er wat gedaan kan worden.
Er moet toch een eindproduct komen.
We zullen dus met een meer generieke variant calling tool bezig gaan.
Lofreq <!-- TODO vraag jasper permissie -->
...
Deze lijst kan je downloaden, maar er staat veel overbodige informatie in.
Daarom heb ik een scriptje in python geschreven die de genen er uit kan halen.
zie main src/python.
...
<https://github.com/pcingola/SnpEff/issues/536#issuecomment-2189201996> Dankjewel lukedow.

Daarna heb ik snpeff weer uitgevoerd.
Maar toen ontstond er het volgende probleem, de chromosoom benaming in de vcf bestanden en de net gebouwde data base komen niet overeen.
Ik hen online op ncbi een mapje gevonden die de ncbi benaming omzet naar chromosome nummers, zie etc/chromosome_map.txt
...
Hierna kreeg ik de volgende foutmelding, geen idee wat het inhoud maar er was een quick workaround voor 'Contig 'NC_000067.7' is not defined in the header.
...
Dit werkt ook niet: 'Contig 'NC_000067.7' is not defined in the header.
(Quick workaround: index the file with tabix.'
...
In deze tabel staan een aantal NT\_\*\*\* chromosomen, die ik niet ken en niet voorkomen in mijn chromosome map.
Morgen zal ik hier verder naar kijken.
...
De UN staat dus voor unkown gedeelte, en kan dus niet gemapt worden aan een bestaand chromosoom.
Deze veroorzaakt ook de CHROMOSOME_Not_found errors.
...
Waarschijnlijk hebben we een BED file nodig met chromsomen, exon posities en genen voor de visualisatie.
Die hebben wij op dit moment niet, dus ik zal deze voor het mm39 muis referentie genoom genereren.
...
Ik blijf de vorige gebruiken.
De waarschuwingen kunnen voorkomen wanneer het genoom niet helemaal gebrepen is, en dat kan aangezien we ook Unkown chromosome maps hebben.
...
Het python3 scriptje staat ook weer in src/python.
Met dit scriptje kan ik een gene.txt bestand meegeven waar gezochte genen in staan.
Het scriptje zal deze genen dan zoeken in het VCF bestand, en een VCF output geven met de lines die het gen bevatten.
...
Tot nu toe zijn alle SNPeff testen gedaan in mijn temp folder, ik ga het nu uitvoeren voor de ouputs folder.
Morgen zal ik dan met het groepje verder sorteren en filtreren.
...
Daarom gaan wij gericht zoeken naar genen die belangrijk zijn bij processen waar blaas kanker kan ontstaan Ik heb daarvoor zoals eerder benoemd een bestand voor gedownload met alle genen.
<https://www.informatics.jax.org/mgihome/homepages/help.shtml>

Deze kan ik met mijn eigen scriptje omzetten naar een lijst met genen.
Deze kan ik als input gebruiken om die genen te vinden in de vcf bestanden.
...
Voordat ik de VCF bestanden verder kan verwerken moet ik deze valideren.
Dit zal ik eerst doen op de bestanden zonder kanker gen filter
...
Hier uit komt "Cannot find header for INFO field 'ANN'", wat inhoud dat ik in mijn python3 script aan moet passen zodat deze ook de headers meeneemt.
Ik zal deze nu aanpassen, zodat ##INFO header ook in het output bestand komt te staan
...
En het vervolgens weer valideren met hetzelfde command als hiervoor.
Hoewel deze validator zegt dat mijn gegenereerde data wel valide is, kan ik met bcftools stats geen output krijgen.
Ik zal proberen alle headers toe te voegen, om te kijken of deze output wel valide is
...
Ook bcftools stats werkt nu op deze bestanden.
Zo kunnen we met vcfstats straks plotjes genereren waarmee we ook kunnen spreken over wat de vcf bestanden betekenen en vertellen.
Hiervoor wordt dus vcfstats gebruikt, en python package die plotjes met vcf data kan genereren.
...
De output hiervan is het aantal varianten per chromosome voor dat vcf bestand.
Als ik dit nog een keer zou uitvoeren op het bestand die op kanker genen is gefilterd kunnen die 2 plotjes vergeleken worden.

## 08-10-2024

Tabix genereren voor Iris, eerst vcf filter opnieuw uitvoeren omdat er per ongelukt duplicaten in de output zijn gekomen.
Dus het scriptje is nu aangepast
...
Nu met tabix een index genereren.
Deze hebben we nodig om een plotje te maken.
...
Wij hebben eerder een bed bestand gegenereerd met een python scriptje, deze bevat *alle* genen in een muis.
Ook hebben wij een gene panel opgesteld die de genen bevat die wij waardevol vinden.
...
Nu hebben we een simpel plotje van varianten op chromosome 1.
Met allel frequentie op de y-as Graag wil ik de exon locaties laten zien van een gezocht gen.
...
Ons VCF bestand mist headers die wij met een galaxy tool uit de ANN kunnen halen.
SnpSift extract Fields.
Zo krijgen we straks de gen namen als header, en ook de impact van de mutatie.
...
for (gene_vcf in unique(snp_data$ANN....GENE))
{
    ...
    range <- IRanges(start = snp_data$POS[snp_data$ANN....GENE == gene_vcf],
                    end = snp_data$POS[snp_data$ANN....GENE == gene_vcf],
                    names = gene_vcf)
    grange <- GRanges(seqnames = unique(snp_data$CHROM[snp_data$ANN....GENE == gene_vcf]),
                      ranges = range)


    list_variant_ranges[[gene_vcf]] <- grange
...
Dit geeft een lijst met alle genen met een variant in dat gen
Hier kan nu 1 uit gekozen worden en een loliplot van gemaakt worden
...
SNPs <- unique(start(list_variant_ranges[[gene]]))

chromosome <- unique(snp_data$CHROM[snp_data$ANN....GENE == gene])

height_map <- c("LOW" = 1, "MODIFIER" = 5, "MODERATE" = 10, "HIGH" = 15)
sample.gr <- GRanges(chromosome, IRanges(SNPs, width = 1, names = paste0("snp", SNPs)))

sample.gr$impact <- snp_data$ANN....IMPACT[snp_data$POS %in% SNPs]


sample.gr$score <- height_map[sample.gr$impact]


sample.gr$color <- sample.int(10, length(SNPs), replace = T)
...
```
De gebruikte galaxy tool zorgt voor duplicaten omdat er meerdere dingen uit de info kolom gehaald kunnen worden.
Deze worden telkens op een nieuwe regel geplaatst, er kan op 1 positie maar 1 variant zijn. Dus wordt unique gebruikt om duplicaten weg te halen.

Ons bed bestand is gegenereerd met een snel python scriptje van een GTF bestand, en de meeste exonen hebben dus overlap. Er blijkt ook een tool te zijn op de galaxy server die GFF omzet naar bed, deze zal ik nu gaan proberen.
Het bed bestand die hier uitkomt bevat niet de namen van de genen, die hebben we wel nodig.

Er is ook nog een gtf to bed tool, deze zal ik ook gaan testen om te kijken of die gebruikt kan worden.
Ook deze maakt een bed file zonder de gen namen er in te zetten.

Ik blijf voor deze opdracht dus het zelf gemaakte bed bestand gebruiken, ookal is deze minder accuraat.

In het plotje is het volgende zichtbaar:
op de x as de positie op een chromosoom, met daarbij de exonen van het gezochte gen die komen uit het bed bestand. De lolipops geven weer waar een mutatie is gevonden, en de hoogte van de loli geeft weer wat voor impact de mutatie heeft op het gen.
Zo zien we in het lange exon dat er 5 mutaties zijn gevonden waarbij 2 mutaties een lage impact heeft op het gen, en 3 een moderate impact.
