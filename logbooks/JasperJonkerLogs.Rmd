...
Dit is het Logboek van Jasper Jonker voor het vak 2.1.2 Genomics & Transcriptomics. Met dit vak gaan we een paper opnieuw na doen of een andere onderzoeksvraag beantwoorden met de data die gegeven is bij de paper.

## 09-09 

We starten het onderzoek. We moeten eerst een paper vinden of verslag met genoeg meetwaarde heeft met meerde meetwaarde onder de zelfde omstandigheden. We doen het onderzoek met vier mensen dus hebben besloten om ieder 1 uit te kiezen. Om zeker te weten dat er 1 tussen zit die vol doet aan alle eizen die gesteld worden en we van alle onderzoeken kunnen verkleinen naar maar vier. Daar uit is het voor ons dan makkelijker uitkiezen.

## 10-09 

Ik heb er gekozen voor het volgende onderzoek https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE153650 . Het is een onderzoek dat gaat over het bestrijden van blaaskanker met behulp van specifiek soort eiwitten. Omdat er hedendaags nog heel veel mensen dood aan gaan en ook de behandeling die ze nu gebruiken, dus chemotherapie, niet altijd even goed aan slaat. Dit onderzoek wordt eerst op muizen getest. Nu nog wachten op goedkeuring van de docenten. 
...
Bezig met de worklfow en gekeken wat we uiteindelijk gaan doen. We hebben besloten om de het onderzoek na te bootsen maar ook een beetje te verbreden. We gaan ook kijken wat de mutatie van de kanker en wat voor gevolg het heeft op biologische processen verder in het lichaam. Dat lijkt ons leuker om te doen dan het onderzoek precies na te doen.

## 16-09

Het plan van aanpak gepresenteerd. We hebben ook gelijk gekozen om met het nieuwste muizen gen te gaan vergelijken. Dit omdat dat gen het nieuwst is en ook het meest van bekentst is. Misschien komen er hier door ook op een andere conclusie dan als we het met het oude gen gingen vergelijken.  We hebben ook een taak verdeling gemaakt. Ik ga bezig met bwa omdat het mij iets moeilijks lijkt en ik mezelf wil uitdagen. BWA wordt ook veel gebruikt in de bioinformatica  dus is zoiezo handig om te weten hoe het werkt. BWA is een alining tool tegen over een refence genoom. Wat belankrijk is in veel onderzoeken omdat je dan kan weten of je data ook wel overeen komt met het organisme waar je onderzoek van doet. 

## 17-09

Ik ga vandaag eerst testdata maken in een oefen Folder die later weer wordt verwijderd. We maken test data omdat die data kleiner is dan de normale data en daarom makkelijker te gebruiken is in onze tools zodat het niet te lang duurt. We hebben maar een bepaalde tijd en als we telkens moeten wachten op dat de tools klaar zijn en het fout kan gaan op het laatste moment fout kan gaan gooi je veel belangrijke tijd over. Dit doen we even allemaal en doen het met 10000 reads omdat het ons groot genoeg lijkt. We doen het allemaal omdat het in andere onderzoeken ook gebruikt kunnen worden.
```{bash, eval=FALSE}
...
Hier ga ik ook een Fastqc maken van de oefendata zodat we dat ook allemaal even weten. Want later moeten we Fastqc bestanden maken van de data om te kijken of het na het trimmen de data naar wens is met niet te veel duplicaten bijvoorbeeld en of de data wel van goede kwaliteit is.
```{bash, eval=FALSE}
...
Vandaag ga ik bezig met BWA terwijl andere bezig zijn met het trimmen hier door werken we alvast een beetje voor uit en kunnen we, als we klaar zijn met trimmen kunnen we in 1 keer door. Ik doe het eerst simpel en in een testingtools map zodat het duidelijk is voor iedreen die het dan leest en ik het ook goed kan uitleggen.
``` {bash, eval= FALSE}
...
## 20-09 
Bezig geweest met trimmiotic, er is na het trimmen kwaliteit om laag gegaan en dat is niet logisch. Onze ongetrimde data was zo ie zo al super goed dus we hebben ook gelezen of ze het al getrimmed hebben geüpload maar we kunnen er niks over vinden. Dus we gaan eerst nog verder kijken naar wat handig is om te doen. Kijken of we het op een bepaalde manierwel kunnen trimmen of we het op de ongetrimmde data gewoon verder gaan en dan kijken het het wel gaat.
...
cat /students/2024-2025/Thema05/BlaasKanker/SRR_Acc_List.txt | \
  parallel 'TrimmomaticPE -threads 4' \
                  ...
                  'MINLEN:40'
```
Mijn terminal doet raar. Mijn webpagina sluit telkens af en is super sloom dus het gaat niet helemaal lekker. Ik ga eerst wel veder met lezen wat we allemaal kunnen doen met trimmen zodat onze data wel goed er uit komt

We kunnen het proberen met traling toe te passen. Dit verwijderd de laatste paar basen verwijderd worden totdat een totdat er een kwaliteit gevonden is die gewenst is.
Hetzelfde geld voor leading. Maar dat is dan bij de eerste paar basen met te lage kwaliteit. 
Min len kunnen we ook gebruiken. Die halen de sequencies er uit die te klein zijn bij een bepaalde lengte.
...
```
Het werkt zoals het zou moeten en nu even wachten op het trimmen. Hier hebben we no twijfels over wat we moeten doen met de duplicaten en gaan we dan op
25-09 gaan we het mappen op de echte getrimmde data. 
...
Ik was de laaste stappen vergeten toe te voegen maar het werkt nu als het goed is
...
oefen data maken voor iris voor twee bestanden. Zodat zij verder kan werken en het niet perse op echte data het moet proberen
```{bash, eval=FALSE}
cat /students/2024-2025/Thema05/BlaasKanker/SRR_Acc_List.txt | \
  parallel -j 20 "bwa mem -t 15 -M -R '@RG\\tID:{}\\tSM:{}\\tPL:illumina\\tLB:lib1' \
    /students/2024-2025/Thema05/BlaasKanker/outputs/bwa_aln/index/GCF_000001635.27_GRCm39_genomic.fna \
...
```
## 27-09 
ik heb de data nu goed gemaakt was blijkbaar niet geod gegaan waarschijnlijk door de slechte servers. Want ik kreeg geen fout meldingen of wat dan ook  

Ik ga nu weer nieuwe testdata maken van alle testdata. zodat iris er mee bezig kan. Die heeft mijn data nodig om haar tool te kunnen gebruiken.
```{bash, eval=FALSE}
cat /students/2024-2025/Thema05/BlaasKanker/SRR_Acc_List.txt | \
...
## 04-10
Ik ga GSEA bekijken en downloaden. GSEA is een tool waar je mee kan kijken of vooraf gedefinieerde set genen significant, overeenstemde verschillen heeft onder 2 verschillende biologische processen. Hier mee kunnen we kijken wat voor invloed de mutaties hebben. We gaan het nu nog niet gebruiken. Dat komt omdat we nu nog niet weten wat de bedoeling is met de eind bestanden die we hebben gemaakt. 

Nu ga ik dus bezig met vcfstats. Dit om de vcf bestanden te laten zien.
...
```
Hier onder zie je de twee figueren van hetzelde sample maar de linker zijn alle varianten en de rechter zijn alle varianten die kanker veroorzaken.
```{r echo=FALSE}
library(cowplot)
library(ggplot2)
...
Hier kan je zien dat er op het chromosoom 7 de meeste zijn die kanker veroorzaken. Omdat het zoveel is gaan we later nog filteren op hoe erg de invloed is van de mutatie op het biologisch proces en pakken we de gene met een een groot gevolg. 
